...
@property (nonatomic ) dispatch_queue_t prepareFilterQueue;
...
- (NSURL *)fileURLForFinalMixedAsset {
    ...
    @synchronized(self) {
        if (tempMixedAssetURL == nil) {
            tempMixedAssetURL = [NSURL fileURLWithPath:[NSString stringWithFormat:@"%@%@", NSTemporaryDirectory(), @"tempMix.mov"]];
        }
    }
...
- (void)combineSoundAndMovie {
    ...
    [self.assetExportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        switch (self.assetExportSession.status) {
            ...
            default: {
                 BOOL yn = [[NSFileManager defaultManager] fileExistsAtPath:self.assetExportSession.outputURL.path];
                 break;
            }
        };
...
- (NSURL *)fileURLForTempMovie {
    ...
    @synchronized(self) {
        if (tempMoviewURL == nil) {
            tempMoviewURL = [NSURL fileURLWithPath:[NSString stringWithFormat:@"%@%@", NSTemporaryDirectory(), @"temp.mov"]];
        }
    }
...
- (void)takePhoto {
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        @autoreleasepool {
            ...
            image = [image cropImageWithBounds:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH)];
...
- (void)switchToNewFilter {
    ...
    if ([self.delegate respondsToSelector:@selector(IFSwitchFilterFinishToImage:)]) {
        [self.delegate IFSwitchFilterFinishToImage:[[self.filter imageFromCurrentlyProcessedOutput] imageRotatedByDegrees:0]];
    }
